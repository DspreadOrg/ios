...
-(NSString *)checkAmount:(NSString *)tradeAmount{
    ...
    if (![DSPUtil isPureInt:tradeAmount]) {
        return rs;
    }
...
-(void)onRequestCalculateMac:(NSString *)calMacString{
    ...
    //    NSData *aa = [DSPUtil stringFormatTAscii:calMacString];
    //    NSLog(@"aaaaa: %@",[DSPUtil byteArray2Hex:aa]);
    NSString *msg = @"Replied success.";
    msgStr = @"Request data to server";
    [self conductEventByMsg:msgStr];
    //    mAlertView = [[UIAlertView new]
...
-(void)UpdateEmvCfg{
    NSString *emvAppCfg = [DSPUtil byteArray2Hex:[self readLine:@"kernel_app_finexus"]];
    NSString *emvCapkCfg = [DSPUtil byteArray2Hex:[self readLine:@"capk_finexus"]];
    [pos updateEmvConfig:emvAppCfg emvCapk:emvCapkCfg];
}
- (IBAction)updateEmvCfg:(id)sender {
//    NSString *emvAppCfg = [DSPUtil byteArray2Hex:[self readLine:@"kernel_app_"]];
//    NSString *emvCapkCfg = [DSPUtil byteArray2Hex:[self readLine:@"capk_"]];
//    
//    [pos updateEmvConfig:emvAppCfg emvCapk:emvCapkCfg];
...
- (IBAction)getPosInfo:(id)sender {
    ...
    self.textViewLog.text = @"starting...";
    [pos doUpdateIPEKOperation:@"00" tracksn:@"FFFF000000BB81200000" trackipek:@"C71A5183A4CF3D969F4C28CD8E170266" trackipekCheckValue:@"39E22B635569AB31" emvksn:@"FFFF000000BB81200000" emvipek:@"C71A5183A4CF3D969F4C28CD8E170266" emvipekcheckvalue:@"39E22B635569AB31" pinksn:@"FFFF000000BB81200000" pinipek:@"C71A5183A4CF3D969F4C28CD8E170266" pinipekcheckValue:@"39E22B635569AB31" block:^(BOOL isSuccess, NSString *stateStr) {
        if (isSuccess) {
            self.textViewLog.text = stateStr;
        }
    }];
...
//    }];
    //[pos resetPosStatus];
//    NSString *app =[DSPUtil byteArray2Hex:[self readLine:@"kernel_app_finexus"]];
//    NSString *capk =[DSPUtil byteArray2Hex:[self readLine:@"capk_finexus"]];
...
//  }];
    ...
    //    NSString *a = [DSPUtil byteArray2Hex:[DSPUtil stringFormatTAscii:@"622526XXXXXX5453"] ];
...
-(void)calcMacDouble:(NSString *)cal{
    NSData *aa =  [DSPUtil ecb:[DSPUtil HexStringToByteArray:cal]];
    NSLog(@"aa = %@",aa);
    [pos calcMacDouble_all:[DSPUtil byteArray2Hex:aa] keyIndex:0 delay:10];
}

-(void)calcMacSingle:(NSString *)cal{
    NSData *aa =  [DSPUtil ecb:[DSPUtil HexStringToByteArray:cal]];
    NSLog(@"aa = %@",aa);
    [pos calcMacSingle_all:[DSPUtil byteArray2Hex:aa] delay:10];
}
...
- (IBAction)sendApdu:(id)sender {
    ...
    /*[pos sendApdu:@"00A404000CA00000024300130000000101" block:^(BOOL isSuccess, NSData *result) {
        if (isSuccess) {
            weakSelf.textViewLog.text = [DSPUtil byteArray2Hex:result];
